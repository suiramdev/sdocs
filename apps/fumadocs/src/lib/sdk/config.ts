...
export const sdkConfig = {
  ...
  data: {
    entitiesDir: path.join(appRootDir, "data", "sdk", "entities"),
    ...
    rootDir: path.join(appRootDir, "data", "sdk"),
  },
  docsBaseUrl: "/docs/sdk",
  meilisearch: {
    apiKey: process.env.MEILI_API_KEY ?? "sdk-dev-master-key",
    ...
    indexName: process.env.MEILI_SDK_INDEX_NAME ?? "sdk_entities",
  },
} as const;

export const clampSemanticRatio = (value: number | undefined): number => {
  if (value === undefined || Number.isNaN(value)) {
    return sdkConfig.meilisearch.defaultSemanticRatio;
  }
