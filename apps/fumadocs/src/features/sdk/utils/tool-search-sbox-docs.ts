...
} from "@/features/sdk/utils/sbox-search";
import type { SboxDocsSnippet } from "@/features/sdk/utils/sbox-search";

export const searchSboxDocsTool = {
  description:
    "Search s&box SDK docs and return structured snippets with signatures, descriptions, and example usage.",
  input_schema: {
...
const buildSearchUrl = (input: SearchSboxDocsToolInput): URL => {
  const url = new URL("/api/sbox/search", sdkConfig.app.baseUrl);
  const pairs = [
    ["q", input.query],
