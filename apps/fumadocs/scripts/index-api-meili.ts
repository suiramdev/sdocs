...
import type { ApiEntity } from "../src/features/api/utils/schemas";
...
async function main() {
  ...
  const indexName = process.env.MEILI_API_INDEX_NAME ?? "api_entities";
  ...
  const index = client.index<ApiEntity>(indexName);
