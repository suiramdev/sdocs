...
const apiDocsRoot = path.join(projectRoot, "content", "api-generated");
const apiDataRoot = path.join(projectRoot, "data", "api");
const entitiesRoot = path.join(apiDataRoot, "entities");
...
function parseArgs(argv: string[]): CliOptions {
  ...
  if (typeof input !== "string") {
    throw new TypeError(
      "Missing --input. Example: bun run scripts/generate-api-docs.ts --input /path/to/api.json"
    );
  }
...
function buildMeiliId(entityId: string, sequence: number): string {
  return `api_${sequence}_${shortHash(entityId)}`;
}
...
function toDocUrl(filePath: string): { pathValue: string; url: string } {
  const relativeToApiRoot = path
    .relative(apiDocsRoot, filePath)
    .replaceAll("\\", "/");
  const pathWithoutExtension = relativeToApiRoot
    .replace(/\/index\.mdx$/u, "")
    .replace(/\.mdx$/u, "");

  return {
    pathValue: `api/${relativeToApiRoot}`,
    url: `/docs/api/${pathWithoutExtension}`,
  };
}
...
function renderParameterSection(parameters: ApiParameter[]): string {
...
function renderReturnSection(entity: ApiEntity): string {
...
function renderExceptionsSection(exceptions: ApiException[]): string {
...
function renderRemarksSection(entity: ApiEntity): string {
...
function renderMdxPage(entity: ApiEntity): string {
  const summary =
...
async function main() {
  ...
  const rawDump = JSON.parse(rawContent) as RawApiDump;
  ...
  for (const rawType of types) {
    ...
    for (const property of properties) {
      ...
      entitySequence += 1;
      const propertyEntity: ApiEntity = {
        anchor: "",
        assembly: withFallback(rawType.Assembly, "Unknown Assembly"),
        canonicalUrl: typeEntity.url,
        class: typeFullName,
...
description: ${quoteYaml(`API namespace ${namespaceName}`)}
---
...
`
  ...
  if (options.emitMdx) {
    await writeTextFile(
      ...
      `---
title: "API"
description: "Generated API docs from latest imported JSON."
---
...
`
  ...
  if (options.emitMdx) {
    const apiMetaPath = path.join(apiDocsRoot, "meta.json");
    const existingApiMeta =
      (await loadJsonFile<Record<string, unknown>>(apiMetaPath)) ?? {};
    if (!("title" in existingApiMeta)) {
      await writeTextFile(
        apiMetaPath,
        JSON.stringify(
          {
    ...
    const rootStat = await stat(path.join(apiDocsRoot, "index.mdx")).catch(
      () => null
    );
    if (!rootStat) {
      await writeTextFile(
        path.join(apiDocsRoot, "index.mdx"),
        `---
title: API Search Platform
description: Generated API documentation and AI search routes
---

API MDX files are generated for offline/reference output under content/api-generated.
`
      );
    }
  }
